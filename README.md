...
«15.1 Магические методы», «16.1 Наследование», «16.2 Множественное наследование» и «17.1 Исключения».
...
- абстрактный базовый класс `BaseProduct` для всех типов товаров;
- логирующий `CreationInfoMixin` в цепочке наследования `Product`;
- автоматический вывод класса и параметров при создании каждого товара;
- дополнительный класс `Order` для покупки одного вида товара;
- общий абстрактный класс `BaseEntity` для `Category` и `Order`;
- запрет создания товара с нулевым количеством через `ValueError`;
- пользовательское исключение `ZeroQuantityError` для категории и заказа;
- уведомления об успешном добавлении товара и завершении его обработки;
- расчёт средней цены товаров категории через `Category.middle_price()`;
- возврат нуля при расчёте средней цены пустой категории;
...
При программной попытке снизить цену товара сеттер запросит подтверждение: `y` применяет новую цену, любой другой
ответ отменяет изменение.
...
Фактическое покрытие функционального кода составляет **99%** (63 теста). Подробный результат сохранён в
...
- `feature/homework-17-1` — полностью выполненное задание 17.1.

Для сдачи работы нужно создать Pull Request из `feature/homework-17-1` в `develop`.
